...
const urlDetail = 'https://api.rawg.io/api/games/';

//Modal API request
const modalRequest = async (id) => {

    const url = `${urlDetail}${id}?key=${key}`;
    ...
    fetch(url, {
        ...
        then(async (resp) => {
            ...
            if (resp.status === 200) {
                ...
                loadModal(JSON.parse(JSON.stringify(respJson)));
...
const openModal = (id) => {
    try {
        ...
        console.log(error);
        modalView.innerHTML = '<p>Could no connect with the API for detailed information. Sorry.</p>';
    }
}
